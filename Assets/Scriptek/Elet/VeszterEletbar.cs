...
public class VeszterEletbar : MonoBehaviour
{
    [SerializeField] private Veszter veszter;      // Reference to the Veszter (mini-boss) component
    ...
    {
        if (veszter != null && jelenlegielet != null)
        {
            // Set fill amount based on current health divided by 10 (to match the 10 hearts)
            jelenlegielet.fillAmount = (float)veszter.Health / 10f;
