...
public class BocsEletbar : MonoBehaviour
{
    [SerializeField] private Bocs bocs;      // Reference to the Veszter (mini-boss) component
    ...
    {
        if (bocs != null && jelenlegielet != null)
        {
            // Set fill amount based on current health divided by 10 (to match the 10 hearts)
            jelenlegielet.fillAmount = (float)bocs.Health / 10f;
